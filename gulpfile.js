 ...
 * 上线编译任务
 */
gulp.task('css',['concatJs'],function(cb) {
    console.log('css rev');
...
gulp.task('build',['rev'],function(cb) {
    ...
    gulp.src(['./build/favicon.ico']).pipe(gulp.dest('./public')); //
    gulp.src(['./build/upload']).pipe(gulp.dest('./public')).on('end' , cb); //
});